...
abstract class FlameGame extends Game
  ...
  void onTap() {
    components
        .where((c) =>
            ((c is GameComponent) && c.gestureHandlable(GestureType.Tap)))
        .forEach((c) => (c is GameComponent) ? c.onTap() : null);
  }

  // @override
  // void onDoubleTap() {
  //   components.forEach((c) => c.onDoubleTap());
  // }
  ...
  void onLongPressStart(LongPressStartDetails details) {
    components
        .where((c) =>
            (c is GameComponent) &&
            c.gestureHandlable(GestureType.LongPress) &&
            c.offsetInArea(details.globalPosition))
        .forEach(
            (c) => (c is GameComponent) ? c.onLongPressStart(details) : null);
  }
  ...
  void onPanUpdate(DragUpdateDetails details) {
    components
        .where((c) =>
            (c is GameComponent) &&
            c.gestureHandlable(GestureType.PanUpdate) &&
            c.offsetInArea(details.globalPosition))
        .forEach((c) => (c is GameComponent) ? c.onPanUpdate(details) : null);
  }
  ...
  void onTapDown(TapDownDetails details) {
    components
        .where((c) =>
            (c is GameComponent) &&
            c.gestureHandlable(GestureType.TapDown) &&
            c.offsetInArea(details.globalPosition))
        .forEach((c) => (c is GameComponent) ? c.onTapDown(details) : null);
  }
}
